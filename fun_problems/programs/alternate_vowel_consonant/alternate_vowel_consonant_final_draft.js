...
function checkRemaining(word, resultWord) {
    ...
    return isVowelOrNot ?
      vowelPosition(word, 0, resultWord, '') :
      consonantPosition(word, 0, resultWord, '');
  }
}

function vowelPosition(word, startIndex, resultWord, remainingLetters) {
  ...
  for (let index = startIndex; index < word.length; index++) {
    if (isVowel(word[index])) {
      ...
      index = index + 1;
      return consonantPosition(word, index, resultWord, remainingLetters);
    } else {
...
function consonantPosition(word, startIndex, resultWord, remainingLetters) {
  if (startIndex === word.length && remainingLetters.length === 0) {
    resultWord = resultWord + ',' + remainingLetters;
    return remvoeCommaFromEnd(resultWord);
  }
  ...
  for (let index = startIndex; index < word.length; index++) {
    if (!isVowel(word[index])) {
      resultWord = resultWord + word[index];
      index = index + 1;
      return vowelPosition(word, index, resultWord, remainingLetters);
    } else {
      remainingLetters = remainingLetters + word[index];
    }
  }
