...
function isOut(choices, isBatting, batter) {
  const isInValidUserChoice = !VALID_SCORE.includes(choices[batter]) && isBatting;
...
function calculateScore(isUser, userBatting, target = 37, score = 0, noOfBallsLeft = 6) {
  if (noOfBallsLeft === 0) {
    return score;
  }
  ...
  runMessage();
  return calculateScore(isUser, userBatting, target, latestScore, noOfBallsLeft - 1);
}
